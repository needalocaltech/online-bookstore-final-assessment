...
def calculate_discounted_price(price, fixed_discount=None, percentage_discount=None):
    """
    Calculates discounted price based on fixed amount or percentage.
    Example:
        calculate_discounted_price(100, fixed_discount=10) -> 90
        calculate_discounted_price(100, percentage_discount=0.2) -> 80
    """
    if fixed_discount is not None:
        discounted_price = price - fixed_discount
    elif percentage_discount is not None:
        discounted_price = price * (1 - percentage_discount)
    else:
        discounted_price = price

    # Round to two decimals for currency formatting
...
def apply_discount(amount: float, code: str) -> float:
    ...
    # The original models.py had no definition for 'calculate_discounted_price',
    # so we'll revert to the original logic here for codes that don't match,
    # as the current version of apply_discount doesn't call the main helper
    # for no-match cases.
